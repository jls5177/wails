...
func GetWailsVersion() (*semver.Version, error) {
	...
	// Find wails version
	versionRegexp := regexp.MustCompile(`.*github.com/jls5177/wails.*(v\d+.\d+.\d+(?:-pre\d+)?)`)
...
func UpdateGoModVersion() error {
	...
	requireLine := "-require=github.com/jls5177/wails@v" + currentVersionString

	// Issue: go mod edit -require=github.com/jls5177/wails@1.0.2-pre5
